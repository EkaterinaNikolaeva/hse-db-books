...
from 
    bookmetrics.shop s
join 
    bookmetrics.booking b on s.shop_id = b.shop_id
join 
    bookmetrics.book_in_booking bib on b.booking_id = bib.booking_id
join 
    bookmetrics.book_in_shop bis on bib.book_in_shop_id = bis.record_id
group by 
    s.shop_id, s."name";

SELECT * from bookmetrics_view.shop_sales_summary;


-- Отображает все брони пользователя, в случае если броней несколько,
-- показывает их все в разных строках.
-- Включает id и имя пользователя, id и дату брони, id и название магазина,
-- а так же количество забронированных книг и их список
...
SELECT * from bookmetrics_view.active_bookings;
...
join 
    bookmetrics.author_x_book axb on a.author_id = axb.author_id
join 
    bookmetrics.book b on axb.book_id = b.book_id
join 
    bookmetrics.book_in_shop bis on b.book_id = bis.book_id
join 
    bookmetrics.book_in_booking bib on bis.record_id = bib.book_in_shop_id
group by 
    a.author_id, a."name"
order by 
    total_books_sold desc, total_revenue desc;

SELECT * FROM bookmetrics_view.author_popularity;